...
namespace todo_aspnetmvc_ui.Contexts
{
    /// <summary>
    /// A class to identify a mobile or desktop browser.
    /// </summary>
    public class TypeBrowser
    {
        private readonly HttpContext _httpContext;
        private readonly Regex _mobileCheck = new Regex(
            @"(android|bb\d+|meego).+mobile|avantgo|bada\/
            |blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo
            |midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker
            |pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino",
            RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);

        private readonly Regex _mobileVersionCheck = new Regex(
        ...
        /// </summary>
        /// <param name="httpContext">HTTO-specific information.</param>
        public TypeBrowser(HttpContext httpContext)
        {
            _httpContext = httpContext;
        }

        /// <summary>
        /// Detect type browser, if mobile return true otherwise false.
        /// </summary>
        /// <returns>True - mobile, false - desktop.</returns>
        public bool IsMobileDeviceBrowser()
        {
            var u = _httpContext.Request.Headers[HeaderNames.UserAgent].ToString();

            const int LENGTH_REGEX = 4;

            bool isMobileBrowser = false;
            if (u.Length > 4 && (_mobileCheck.IsMatch(u) || _mobileVersionCheck.IsMatch(u[..LENGTH_REGEX])))
            {
